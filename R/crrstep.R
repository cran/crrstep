        ...
        if (backward) {
            ...
            if (o[1] == 1) {
                ...
                outmat <- cbind(signif(fit$coef, 3), signif(std.error, 
                  3), round(abs(fit$coef)/std.error, 3))
            ...
            if (!is.na(var)) {
                cov3.2 <- cov3.2[, -c(grep(var, colnames(cov3.2))), 
                  drop = FALSE]
                if (ncol(cov3.2) != 0) {
            ...
            if (!is.null(fit)) {
                outmat <- cbind(signif(fit$coef, 2), signif(std.error, 
                  2), signif(abs(fit$coef)/std.error, 3))
            ...
            if (trace) {
               cat("\nStep: ", criterion, "= ", format(round(crr.extractAIC(fit, cov3.2, k),2)), "\n", cut.string(deparse(as.vector(fit$call))), "\n\n", sep="")              
                utils::flush.console()
            }
        }
        if (forward) {
            ...
            formula1 <- as.formula(paste(scope.min[1], scope.min[2], 
                ans, sep = ""))
            scope.min <- formula1
            cov2 <- cbind(cov2, cov1[, c(grep(var1, colnames(cov1))), 
                drop = FALSE])
            ...
            if (length(scope.new2) == 0) 
                break
            if (trace) {
              cat("\nStep: ", criterion, "= ", format(round(crr.extractAIC(fit, cov3.1, k),2)), "\n", cut.string(deparse(as.vector(fit$call))), "\n\n", sep="")              
                utils::flush.console()
            }
        }
    }
    if (crr.object) return(fit) 
    cat("\n")
    print(list(coefficients = outmat, log.likelihood = round(fit$loglik, 2)))
}
